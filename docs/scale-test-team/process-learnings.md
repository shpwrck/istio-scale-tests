...
## PL38 — A best-effort/optional sub-step that can `die` internally is a whole-run killer under `set -e`; its `|| warn` path is dead code unless the step is subshell-isolated
...
```
The subshell contains the `die`'s `exit` so the orchestrator continues; capturing+echoing stderr preserves the real cause. This is the in-process twin of PL32: PL32 wraps per-combo *step scripts* so one failure doesn't kill the sweep; PL38 says an *optional in-process* `die` needs the same containment, and that an unreachable warn path is a silent correctness gap (the feature reads as "best-effort" but is actually fatal). Verify like PL33 — introduce a live failure (point the step at a missing results dir) and confirm the run CONTINUES with the specific reason printed, not that it aborts.

(PL38 was added after the report-hardening cycle: round-1 usability flagged that `003-run-sweep.sh` swallowed `render_scale_envelope` stderr with `2>/dev/null`; fixing it revealed the deeper bug — `render_scale_envelope` calls `die` internally, so without the subshell wrap the "best-effort" warning branch was dead code and a failed envelope render would have aborted the whole sweep after all combos completed. Fixed by subshell-isolating the render and echoing the captured stderr.)
