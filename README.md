...
| `charts/gitops-hub-mesh-ca-intermediate-appset/` | Helm — ApplicationSet in **`openshift-gitops`** + ConfigMap + RBAC; targets **`PlacementDecision`** **`acm-openshift-gitops-placement-decision-1`**; generated apps dest **`openshift-gitops`**.                                                        |
