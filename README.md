...
| `charts/gitops-hub-mesh-ca-intermediate-appset/` | Helm — ApplicationSet in **`open-cluster-management-global-set`** + ConfigMap + RBAC; targets PD **`global-decision-1`**; generated apps dest **`openshift-gitops`**.                                                        |
