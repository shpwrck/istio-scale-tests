...
| `charts/gitops-hub-mesh-ca-intermediate-appset/` | Helm — ApplicationSet in `openshift-gitops` + RBAC; shared PlacementDecision duck-type ConfigMap from `acm-openshift-gitops-resources`; `clusterDecisionResource` + static `in-cluster` list generator; generated apps dest `openshift-gitops`. |
