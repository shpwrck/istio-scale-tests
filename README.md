...
| `charts/gitops-hub-mesh-ca-intermediate-appset/` | Helm — ApplicationSet (`clusterDecisionResource` → `PlacementDecision` **`global-decision-1`**) + generator ConfigMap + RBAC for per-cluster intermediate apps (hub + spokes).                                              |
