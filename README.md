...
**Maintainer note:** When you add or rename numbered scripts under `**istio-setup/`**, `**isotope-multicluster/`**, or optional `**terraform/**/scripts/**`, or change default contexts (`rosa-*`), Istio versions, or manifest paths, update this README (and **AGENTS.md** script naming rule if conventions shift) in the same change so operators stay aligned.
...
- **Platform / mesh pins** (defaults): OpenShift **4.21.11**, Kubernetes **v1.34.6**, Istio `**spec.version`** **v1.28.5**, optional RHACM hub channel `**release-2.16**` — see `**config/versions.env`** (override with env vars when testing other trains). Keep `**ACM_CHANNEL**` aligned with `**OPENSHIFT_VERSION**` using the [RHACM support matrix](https://access.redhat.com/articles/7133095).
...
- `**git`**, Helm 3 — `**istio-setup/001**` installs the RHACM hub from `**charts/acm-hub**`, merges kubeconfig via `**terraform/scripts/001-oc-login-merge-kubeconfig.sh**`, installs `**charts/acm-managed-cluster**` per Terraform spoke, and applies RHACM import YAML on spokes; `**istio-setup/005`** installs `**istio/gateway**` from `**ISTIO_HELM_REPO_URL**` at `**ISTIO_GATEWAY_CHART_VERSION**`.
...
| `**terraform/rosa-hcp/**`                | Optional Terraform stack for **ROSA HCP** (upstream `terraform-redhat/rosa-hcp/rhcs`): **`cluster_count`** plus naming/CIDR **`format()`** strings define **`for_each`** clusters (one VPC and STS stack each). Outputs API URLs and **`cluster_admin_login`**; kubeconfig is operator-managed — see `**terraform/rosa-hcp/README.md`**.                                                                                                                                                                         |
| `**isotope-multicluster/**`              | Multicluster **[istio/tools isotope](https://github.com/istio/tools/tree/master/isotope)** sample topology, `**001-generate-topology-from-terraform.sh**` (chain graph from `**cluster_keys**`), `**002-apply-isotope-multicluster.sh**`. Requires **istio/tools** clone, **Go**, isotope **service** image — see `**isotope-multicluster/README.md**`. |
...
- Bump **script numbers**, **paths**, and **table rows** in this file whenever you add or reorder automation under `**istio-setup/`**, `**isotope-multicluster/**`, or optional `**terraform/**/scripts/**`.
