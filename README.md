...
| `charts/hub-kubeconfig-from-argosecret/` | Helm — External Secrets `SecretStore` + `ExternalSecret` per cluster name; builds `kubeconfig` Secrets from `*-application-manager-cluster-secret` on the hub (ApplicationSet preset `values-kubeconfig-from-argosecret.yaml`). |
