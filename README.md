...
| Step    | Script                                                         | Purpose                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| ------- | -------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **001** | `istio-setup/001-acm-install-hub.sh`                           | **Optional.** Merged kubeconfig from Terraform (unless `--skip-managed-clusters`), then **`charts/acm-operator`** → wait CSV → **`charts/acm-multicluster-hub`** → wait Running → **`charts/acm-klusterlet-config`** (optional) → **`charts/acm-managed-cluster`** + import on spokes. Default `ACM_CHANNEL` pairs with `OPENSHIFT_VERSION` in `config/versions.env`. `ACM_INSTALL_KLUSTERLETCONFIG`, `--skip-managed-clusters`, `--skip-import`, `--skip-wait`, `--dry-run` control behavior. Run first when you want an ACM hub before mesh install. |
