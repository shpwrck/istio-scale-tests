...
| `charts/gitops-hub-mesh-ca-intermediate-appset/` | Helm — ApplicationSet in **`openshift-gitops`** + ConfigMap + RBAC; **`clusterDecisionResource`** + static **`local-cluster`** list generator; generated apps dest **`openshift-gitops`**.                                                        |
