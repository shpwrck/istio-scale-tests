...
PF_PID=""
cleanup_pf() { if [[ -n "$PF_PID" ]]; then kill "$PF_PID" 2>/dev/null || true; wait "$PF_PID" 2>/dev/null || true; fi; }
...
# PL9 / PL21: capture pre-window pilot_info start time from the SAME scrape window.
