...
# Requires: oc, curl
# Usage (repo root):
#   ./setup-scripts/08-ossm-mc-verify-ingress-gateway.sh [--contexts CSV] [--dry-run] [--cleanup]
set -euo pipefail
...
Requires istio-system/istio-ingressgateway (setup-scripts/03) and curl on PATH.
