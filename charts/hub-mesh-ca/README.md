...
Argo CD cannot evaluate Helm **`lookup`** against the hub API. Use **`charts/gitops-hub-mesh-ca-intermediate-appset`**, which combines **`clusterDecisionResource`** (GitOps **`Placement`**, spokes only) with a static **`list`** generator entry **`local-cluster`** for the hub intermediate **`Certificate`**.
