...
The Argo CD instance and the ACM Placement / ManagedClusterSetBinding stay in `openshift-gitops` (`GITOPS_NAMESPACE`).

Installed by Terraform (`terraform/platform/platform_gitops.tf`). Channel defaults from `config/versions.env` (`GITOPS_OPERATOR_CHANNEL`); align with your OpenShift version per [OpenShift GitOps release notes](https://docs.redhat.com/en/documentation/red_hat_openshift_gitops/).
