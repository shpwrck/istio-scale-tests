...
**`hub-mesh-ca-intermediate-appset`** installs the ApplicationSet that generates per-cluster **`hub-mesh-ca-intermediate`** Applications (after **`hub-mesh-ca`** sync wave). Helm destination namespace is **`open-cluster-management-global-set`** so **`clusterDecisionResource`** can resolve **`PlacementDecision`** **`global-decision-1`** there. Override **`applicationSetNamespace`** / **`repo.url`** / **`placement.placementDecisionName`** if your fork or namespaces differ.
