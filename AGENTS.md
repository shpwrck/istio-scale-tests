...
| `config/options.env` | Operational defaults: operator namespaces, GitOps config, mesh/logging defaults, AWS infra, propagation test parameters, and control-plane test parameters (including `CONTROLPLANE_NAMESPACE_COUNT` and `CONTROLPLANE_MAX_MATRIX`). Sourced by `versions.env`; ACM/GitOps defaults are mirrored in `terraform/platform/variables.tf`. |
