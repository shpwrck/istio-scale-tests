...
| `**istio-setup/**`                           | Numbered `**001`–`010**` bash scripts — optional ACM hub (`**001**`), mesh CA (`**002**`), kubeconfig CA prep (`**003**`), templated Istio CRs (`**004**`), Helm ingress gateway (`**005**`), remote secrets (`**006**`), optional insecure API fallback (`**007**`), east–west gateways (`**008**`), verification (`**009**`), optional ingress smoke test (`**010**`). Run from **repo root**; order matters for mesh steps; `**001**` is optional and hub-specific.                                                                                                                                                                                                                            |
...
- `**git**`, **Helm 3** for `**istio-setup/005-ossm-mc-apply-ingress-gateway.sh`** (`**istio/gateway`** chart).
