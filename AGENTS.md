...
- **Shell only:** Mesh setup and repo automation use `**bash`** under `**istio-setup/`**. Do not add Python, Ruby, or other runtimes for install or wiring unless the user explicitly requests it.
...
- **Pinned versions:** Maintain a single pin list in `**config/versions.env`**. Current targets: OpenShift 4.18.38, Kubernetes v1.31.14, Istio / Sail `spec.version` v1.28.5 (match `**istioctl`**), RHACM hub `**ACM_CHANNEL`** default `release-2.15` (supported with OpenShift 4.18.x per RHACM matrix — bump with `**OPENSHIFT_VERSION**`). Bump `**README.md**` when pins change.
...
| `**istio-setup/**`                           | Numbered `**001`–`010**` bash scripts — optional ACM hub (`**001**`, Helm + `**charts/acm-hub**`), mesh CA (`**002**`), kubeconfig CA prep (`**003**`), templated Istio CRs (`**004**`), Helm ingress gateway (`**005**`), remote secrets (`**006**`), optional insecure API fallback (`**007**`), east–west gateways (`**008**`), verification (`**009**`), optional ingress smoke test (`**010**`). Run from **repo root**; order matters for mesh steps; `**001**` is optional and hub-specific.                                                                                                                                                                                                                            |
...
| `**terraform/rosa-hcp/**`                     | Optional Terraform root for **ROSA Hosted Control Plane** clusters via upstream `**terraform-redhat/rosa-hcp/rhcs`** — see `**terraform/rosa-hcp/README.md`**. Mesh install remains under `**istio-setup/`**.                                                                                                                                                                                                                                                                                                                 |
...
- **Paths:** Helpers resolve repo root via `"$(cd "$(dirname "$0")/.." && pwd)"` from `**istio-setup/`** — keep that pattern if you add sibling scripts.
...
- `**git**`, **Helm 3** for `**istio-setup/001-acm-install-hub.sh`** (`**charts/acm-hub**`) and `**istio-setup/005-ossm-mc-apply-ingress-gateway.sh`** (`**istio/gateway`** chart).
