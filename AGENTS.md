...
| `config/options.env` | Operational defaults: operator namespaces, GitOps config, mesh/logging defaults, AWS infra, propagation/controlplane/dataplane test parameters, and O9 scale-coverage sizing/floor knobs (`SCALE_*`, default-off). Sourced by `versions.env`; ACM/GitOps defaults are mirrored in `terraform/platform/variables.tf`. |
