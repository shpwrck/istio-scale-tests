...
| `config/options.env` | Operational defaults: operator namespaces, GitOps config, mesh/logging defaults, AWS infra, propagation/controlplane/dataplane test parameters, O9 scale-coverage sizing/floor knobs (`SCALE_*`, default-off), and the customer SLA verdict bands (`SCALE_SLA_CAUTION_PCT=75` / `SCALE_SLA_FAIL_PCT=90`, read by `tests/lib/envelope.sh:env_sla_verdict` via the controlplane report). Sourced by `versions.env`; ACM/GitOps defaults are mirrored in `terraform/platform/variables.tf`. |
| `charts/spoke-ossm-operator/` | Helm chart: OLM Subscription for Sail operator on each spoke (ApplicationSet wave 8). |
| `charts/spoke-ossm/` | Helm chart: `Istio` + `IstioCNI` CRs per spoke cluster with per-cluster clusterName, network, meshID (wave 21). When the campaign tuning baseline is enabled (`tuningBaseline.*`, default on) it also emits at sync-wave `tuning` (10): a root-ns `Sidecar` (+ two gateway-exempt `workloadSelector` Sidecars so the injected ingress/east-west gateways keep `*/*` egress), two `Telemetry` CRs (client-metric + access-log filtering), and `meshConfig.discoverySelectors` on the `Istio` CR. |
...
| `charts/mesh-cluster-switch/` | Helm chart: mesh-wide traffic switch — root-namespace wildcard `DestinationRule` steering 100% of meshed traffic to a chosen cluster via `localityLbSetting.distribute` (`activeCluster=<region>` \| `balanced`). Requires one-time `topology.kubernetes.io/region` node labels; host-specific DRs shadow it (see chart README). Standalone like `mesh-verify` (not in root app-of-apps). |
| `charts/mesh-wiring-verify/` | Helm chart: always-on per-member wiring health gate. Readiness probe asserts the spoke holds a remote secret per mesh member and istiod has ingested them all (`istiod_managed_clusters` on istiod `/metrics`); NotReady → Argo Application Degraded. Installed by Terraform (`platform_gitops.tf`), which sets `expectedMembers = length(local.mesh_member_spoke_keys)` by name via `template.source.helm.valuesObject.expectedMembers` (order-independent; not a positional `parameters[N]` index — see issue #28). Surfaces partial ESO PushSecret fan-out that Argo (no PushSecret health) and ESO (`Synced=True` on partial push) both hide. Preset: `gitops-hub-ocm-placement-appset/values-mesh-wiring-verify.yaml`. |
...
| `tests/lib/` | Shared bash helper functions sourced by all test suites: `common.sh` (die, split_csv, validation), `timestamp.sh` (portable ns/ms timestamps), `preamble.sh` (harness metadata, TSV preamble, `infra_preamble_lines` shared cluster-infra emitter), `metrics.sh` (Prometheus metric extraction), `capacity.sh` (read-only cluster-capacity probes), `envelope.sh` (campaign scale-envelope render + customer SLA verdict). |
